...
public:
  static void Init() {
    ...
    _packet_handler_map[PKT_CENTERSERVERHANDSHAKE] = [](PacketSessionRef& session, std::byte* buffer, const int32_t len) {
      return HandlePacket<protocol::CenterServerHandshake>(HandleCenterServerHandshake, session, buffer, len);
    };
