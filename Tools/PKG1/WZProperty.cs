...
{
	public static Dictionary<string, Func<WzProperty, string, WzProperty>> _specialUol = new();
	public static Func<WzProperty, string, Task<WzProperty>> _resolveOutlinkHook = (p, v) => p._fileContainer._collection.Resolve(v);
	public List<IDisposable> _alsoDispose;
	public string _name, _nameWithoutExtension, _path;
	...
	public uint _offset;
	public EncryptionType _encrypted;
	public uint _containerStartLocation;
	public long? _pseudoContentStart;

	public ConcurrentDictionary<string, string> _referenceTraces;
	...
	{
		_name = name;
		_nameWithoutExtension = Path.GetFileNameWithoutExtension(name);
		_path = path;
	...
	public IEnumerable<WzProperty> LoadChildren()
	{
		...
		{
			if (_nameWithoutExtension != "zmap" && _nameWithoutExtension == "smap")
				childrenEnumerable = childrenEnumerable.OrderBy(c => c._name);
	...
	public async Task<WzProperty> Resolve(string v = null)
	{
		if (string.IsNullOrEmpty(v) && Type == PropertyType.Uol)
		{
			...
			{
				if (paths.Contains(current._path)) return current;
				paths.Add(current._path);
			}
		...
		if (Children == null)
			if (Package._logging != null)
				Package._logging.LogDebug($"Children null for {_path} ({Type}) ({ResolvesTo}) ({RawData?.Length})");

		var childMatch = Children.FirstOrDefault(c => c._nameWithoutExtension.Equals(childName, StringComparison.CurrentCultureIgnoreCase));
		if (childMatch != null)
		{
		...
		childMatch = Children.FirstOrDefault(c => c._name.Equals(childName, StringComparison.CurrentCultureIgnoreCase));
		if (childMatch != null)
		{
			if (childMatch.Type == PropertyType.Lua) childMatch = await childMatch.Resolve();
			return await childMatch.Resolve(v[Math.Min(firstSlash + 1, v.Length)..]);
		...
		where TK : class
	{
		...
		{
			var link = ret.Children.FirstOrDefault(c => c._nameWithoutExtension.Equals("_inlink", StringComparison.CurrentCultureIgnoreCase)) ?? ret.Children.FirstOrDefault(c => c._nameWithoutExtension.Equals("_outlink", StringComparison.CurrentCultureIgnoreCase)) ?? ret.Children.FirstOrDefault(c => c._nameWithoutExtension.Equals("source", StringComparison.CurrentCultureIgnoreCase));
			if (link != null)
			{
				var linksTo = GetValueAsOrNull<string>(link);
				if (linksTo.StartsWith("Map"))
				{
					if (link._name.Equals("_inlink"))
	...
	public void Dispose()
	{
		...
		if (_alsoDispose != null && _referenceTraces.Count == 0)
		{
			...
			{
				if (PackageCollection._logging != null) PackageCollection._logging.LogDebug($"Disposing of property node ({_path})");
			...
			if (PackageCollection._logging != null)
				PackageCollection._logging.LogDebug($"Property is locked, will dispose later ({_path})");
	}
	...
	private void UnregisterReference(string trace)
	{
		...
		{
			if (PackageCollection._logging != null) PackageCollection._logging.LogDebug($"Disposing of previously disposed property node ({_path})");
