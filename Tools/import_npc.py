...
source_directory = "O:\\Unreal Projects\\MapleStory\\Tools\\NpcExporter\\bin\\Release\\net8.0\\Exported"
...
for subdir, dirs, files in os.walk(source_directory):
    for dir in dirs:
        npc_path = ue_project_directory + "/" + dir

        if dir.isdigit():
            unreal.EditorAssetLibrary.make_directory(npc_path)

        for action_dir, action_name, _ in os.walk(os.path.join(subdir, dir)):
            for action in action_name:
                action_path = npc_path + "/" + action
                source_file_path = os.path.join(subdir, dir, action)
                ue_texture_directory = npc_path + "/" + action + "/Texture/"
                ...
                for file in os.listdir(os.path.join(subdir, dir, action)):
                    if file.endswith(".png") and import_png:
                        ...
                        ue_data_directory = npc_path + "/" + action
        ...
        if imported_count % 100 == 0:
            unreal.EditorAssetLibrary.save_directory(npc_path)
